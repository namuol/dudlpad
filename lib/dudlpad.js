(function() {
  ...
  DUDLPAD.create = function(container, width, height) {
    var canvas, context, currentStyle, drawLines, drawing, hist, resetCanvas;
    if (!(container != null)) {
    ...
    return {
      ...
      lineColor: canHaveCallback(function(color) {
        if (arguments.length === 0) {
          return currentStyle.strokeStyle;
        }
        return currentStyle.strokeStyle = color;
      }),
      lineWidth: canHaveCallback(function(width) {
        if (arguments.length === 0) {
          return currentStyle.lineWidth;
        }
        return currentStyle.lineWidth = width;
      })
    };
  };
}).call(this);
